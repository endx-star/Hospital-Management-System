...
public class Pt_form_pane {
    ...
    public Pane addPatient(int mode) {////1 for add 2 for edit
        ...
        txtId.setText(Integer.toString(InpatientDAO.lastIdGetter("inpatient", "id")));
        ...
        Label label;
        if (mode == 2) {
            label = new Label("Patient's Data Editing Form");// fill the Whole fields.
            label.setFont(Font.font("Rockwell", FontWeight.BOLD, FontPosture.ITALIC, 20));
            label.setTextFill(Color.CORNFLOWERBLUE);
            label.setUnderline(true);
            txtId.setText(Integer.toString(InpatientTableModel.getId()));
            txtRoomNumber.setText(Integer.toString(InpatientTableModel.getRoomNumber()));
            txtAddress.setText(InpatientTableModel.getAddress());
            txtLName.setText(InpatientTableModel.getLastName());
            txtFName.setText(InpatientTableModel.getFirstName());
            txtContactNumber.setText(InpatientTableModel.getPhoneNumber());
            txtGender.getSelectionModel().select(InpatientTableModel.getGender());
            txtBirthDat.setValue(Date.valueOf(InpatientTableModel.getBirthDate()).toLocalDate());
            txtDate.setValue(Date.valueOf(InpatientTableModel.getAdmissionDate()).toLocalDate());
            btnAdd.setOnAction(e -> {
                try {
                    handle_adding_InpatientInfo(1);
                    Form_Window.closeWindow();
            ...
            label = new Label("Patient Registration Form");// fill the Whole fields.
            label.setFont(Font.font("Rockwell", FontWeight.BOLD, FontPosture.ITALIC, 20));
            label.setTextFill(Color.CORNFLOWERBLUE);
            label.setUnderline(true);
            btnAdd.setOnAction(e -> {
                try {
                    handle_adding_InpatientInfo(0);
                } catch (SQLException ex) {
                    ex.printStackTrace();
                }
            });
    ...
    private void handle_adding_InpatientInfo(int status) throws SQLException {
        ...
        if (!InputValidator.isInvalid(tooltips)) {
            ...
            AlertBox.message("You have empty field. All fields are required./n Please fill all fields.");
        } else {
            save_inPatient(status);
            if (status == 1) {
                txtId.setText(Integer.toString(InpatientDAO.lastIdGetter("inpatient", "id")));
                InpatientTableModel inpatientTableModel = new InpatientTableModel();
                Form_Window.stageProvider().close();
    ...
    public void save_inPatient(int state) throws SQLException {
