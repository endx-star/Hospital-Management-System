...
public class PtScene {
    ...
    public Pane content() throws SQLException {
        ...
        search.setOnKeyTyped(e ->
        {
            try {
                ...
                else if (InputValidator.validateNumberType(search.getText().trim(), "process")) {
                    in_table.showSpecificpatient(Integer.parseInt(search.getText()), "");
                } else {
                    in_table.showSpecificpatient(-1, search.getText());
                }
        ...
        btnDelete.setOnAction(e -> {
            try {
                inpatientTableModel.deleteReccored();
            } catch (SQLException ex) {
                ex.printStackTrace();
            }
        });
        ...
        HBox btnLayout = new HBox();///  btn container
        btnLayout.setSpacing(30);
        btnLayout.setPadding(new Insets(10, 5, 5, 5));
        btnLayout.setAlignment(Pos.TOP_RIGHT);
        btnLayout.getChildren().addAll(btnEdit, btnAdd, btnDelete);

        VBox co = new VBox();
        co.getChildren().add( grid);
        co.setAlignment(Pos.TOP_RIGHT);
        co.setSpacing(4);

        HBox content = new HBox();//////contains all functional object
    ...
    public void pt_form_pane(int mode) {
        ...
        } else if (mode == 2) {// edditing state
            if (InpatientTableModel.getId() == 0) {
                AlertBox.selectionChecker("Please select the record  that you want to edit","No entry selected");
