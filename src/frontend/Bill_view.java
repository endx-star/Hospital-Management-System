...
public class Bill_view {
    ...
    public Pane view_billForm() {
        ...
        tipExist = Dr_form_pane.tooltip("This Bill No already exist.It Should be Unique.");
        toolTips = new Tooltip[]{tooltip, tooltip1, tooltip4, tooltip3, tooltip2, tooltips, payedTip, tipExist};
        ...
        bill_no.setOnKeyTyped(e -> {
            tipExist.hide();
            InputValidator.keyTypedNumberHandlerM(tooltips, bill_no, "process");
        });
       ...
       // Button btnEdit = new Button("\t_Edit\t");
        ...
        HBox btnBox = new HBox(btnSave, btnClear);
        btnBox.setAlignment(Pos.CENTER);
        btnBox.setSpacing(10);
        btnSave.setOnAction(e -> saveBillToDatabase(btnSave, bill_no));
    ...
    public void saveBillToDatabase(Button btn, TextField billNo) {
        ...
        if (!billNo.getText().trim().isEmpty()) {
            int bill = Integer.parseInt(billNo.getText().trim());
            if (!BillDAO.billNoChecker(bill)) {
                showTooltip(billNo, toolTips[7]);
                return;
            }
        }
    ...
    public Pane fillInfo() {
        ...
        String bill = (InpatientDAO.columnSelector("total_bill", "bill", "pt_id", l));
